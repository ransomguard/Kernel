...
#define ARW_FS_REG_TAG   'rFRA'   // WinDbg "ARFr" — 레지스트리 조회 임시 버퍼 풀 태그
...
// 언로드 자기보호 킬 스위치. 기본 0(꺼짐). DriverEntry 에서 레지스트리
// Parameters\EnableUnloadProtection 을 1회 읽어 채운다. 1 이면 자발적
// fltmc unload 를 거부한다(강제 언로드는 항상 허용). 언로드 시점에서만
// 참조하므로 pre-write 핫패스와 무관하다.
static volatile LONG   g_ArwFsUnloadProtect = 0;
...
static NTSTATUS ArwFsReadRegistryDword(
...
// ===========================================================================
// 언로드 — 자기보호가 꺼져 있으면 FltUnregisterFilter 후 성공(fltmc unload 가능).
// FltMgr 는 큐잉된 워크아이템이 끝날 때까지 언로드를 진행하지 않으므로,
// 여기서 별도 배수(drain) 처리를 하지 않아도 워커가 참조하는 자원은 안전하다.
//
// [자기보호] EnableUnloadProtection 킬 스위치가 켜져 있고, 이번 언로드가
// '강제(mandatory)'가 아니면 STATUS_FLT_DO_NOT_DETACH 로 거부한다.
//   - fltmc unload: 자발적 언로드 → MANDATORY 플래그 없음 → 거부 가능.
//   - 시스템 종료 / 강제 detach: MANDATORY → 절대 거부하지 않는다.
//     여기서 거부하면 시스템 종료를 방해하게 되므로 반드시 통과시킨다.
// 거부 시 FltUnregisterFilter 를 호출하지 않아 필터가 살아 있고, 이후 강제
// 언로드나 (킬 스위치 해제 후) 재로드에서 정상 정리된다.
// ===========================================================================
_IRQL_requires_max_(PASSIVE_LEVEL)
static NTSTATUS ArwFilterUnload(_In_ FLT_FILTER_UNLOAD_FLAGS Flags)
{
    ...
    if (!FlagOn(Flags, FLTFL_FILTER_UNLOAD_MANDATORY) &&
        InterlockedCompareExchange(&g_ArwFsUnloadProtect, 0, 0) != 0) {
        ARW_FS_LOG("Unload REFUSED by self-protect (EnableUnloadProtection=1). "
            "Set the registry value to 0 and reload to detach.\n");
...
// ===========================================================================
// 레지스트리 DWORD 읽기 — 자기완결(메인 드라이버와 공유하지 않아 분리 유지).
//
// RegistryPath("...\Services\Anti_Ransom_FsFilter") 하위 Parameters 키의 DWORD
// 값을 읽는다. 이 Parameters 키에는 INF 가 Instances/Altitude 를 심어 두므로
// 설정이 한곳에 모인다. PASSIVE 전용. 키/값이 없으면 실패를 반환하고 *Value=0
// 이며, 호출자가 기본값(꺼짐)으로 처리한다. 메인 드라이버 Driver.c 의
// ArwReadRegistryDword 와 같은 ZwOpenKey/ZwQueryValueKey 패턴이다.
// ===========================================================================
_IRQL_requires_(PASSIVE_LEVEL)
static NTSTATUS ArwFsReadRegistryDword(
    _In_  PUNICODE_STRING RegistryPath,
    ...
    _Out_ PULONG          Value)
{
    ...
    *Value = 0;
    ...
    pathBuffer = (PWCH)ExAllocatePool2(POOL_FLAG_PAGED, bufferBytes, ARW_FS_REG_TAG);
    if (pathBuffer == NULL) {
    ...
    info = (PKEY_VALUE_PARTIAL_INFORMATION)
        ExAllocatePool2(POOL_FLAG_PAGED, resultLength, ARW_FS_REG_TAG);
    if (info == NULL) {
...
Cleanup:
    if (info != NULL) {
        ExFreePoolWithTag(info, ARW_FS_REG_TAG);
    }
    if (keyHandle != NULL) {
        ZwClose(keyHandle);
    }
    if (pathBuffer != NULL) {
        ExFreePoolWithTag(pathBuffer, ARW_FS_REG_TAG);
    }
    return status;
}
...
NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT DriverObject, _In_ PUNICODE_STRING RegistryPath)
{
    ...
    // [자기보호 킬 스위치] Parameters\EnableUnloadProtection (REG_DWORD).
    //   기본 0(꺼짐). 1 이면 자발적 fltmc unload 를 거부한다.
    //   값을 못 읽으면(키/값 없음) 꺼진 상태로 둔다. 개발 중 기본 비활성이
    //   원칙이며, 보호가 켜져 잠긴 경우의 탈출은 값을 0으로 바꾼 뒤 재부팅이다
    //   (런타임 변경은 다음 로드까지 반영되지 않음 — SETUP_MINIFILTER.md 참조).
    if (NT_SUCCESS(ArwFsReadRegistryDword(RegistryPath,
            L"EnableUnloadProtection", &regValue))) {
        InterlockedExchange(&g_ArwFsUnloadProtect, (regValue != 0) ? 1 : 0);
    }
    ARW_FS_LOG("Unload self-protect: %s\n",
        InterlockedCompareExchange(&g_ArwFsUnloadProtect, 0, 0) ? "ON" : "OFF");
